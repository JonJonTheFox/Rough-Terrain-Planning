...
class PointCloudVisualizer:
    ...
    def create_legend_elements(self, labels):
        """Create legend elements for the unique labels."""
        ...
        for label in unique_labels:
            ...
            legend_elements.append(Patch(facecolor=color, edgecolor='black',
                                         label=self.metadata_map.get(label, f'Unknown ({label})')))
        return legend_elements

    def plot_pointcloud(self, short_name, index):
        """Plot a 3D pointcloud with colored labels."""
        try:
            ...
            # Add legend
            legend_elements = self.create_legend_elements(labels)
    ...
    def plot_label_distribution(self, short_name, index):
        """Plot the distribution of labels for a specific pointcloud."""
        try:
            _, labels = ih.import_pc_and_labels(short_name, index)
            
            unique_labels, counts = np.unique(labels, return_counts=True)
            label_names = [self.metadata_map.get(label, f'Unknown ({label})') for label in unique_labels]
    ...
    def plot_2d_skyview(self, short_name, index):
        """Plot a 2D skyview of the pointcloud, colored by labels."""
        try:
            points, labels = ih.import_pc_and_labels(short_name, index)
...
def main():
    ...
    index = 2  # Replace with your desired index
